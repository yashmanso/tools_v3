...
export function BookmarkButton({ resource, size = 'md', className = '' }: BookmarkButtonProps) {
  // Start with false to match server render - will update after mount
  ...
  // During SSR and initial client render, always render as unbookmarked to prevent hydration mismatch
  // After mount, the useEffect will update the state based on localStorage
  const displayBookmarked = mounted && bookmarked;
  ...
  return (
    <Button variant="ghost"
      ...
      title={displayBookmarked ? 'Remove from favorites' : 'Add to favorites'}
      aria-label={displayBookmarked ? 'Remove from favorites' : 'Add to favorites'}
      suppressHydrationWarning
    >
      <svg
        className={`${iconSizeClasses[size]} ${isInPanelHeader && !displayBookmarked ? 'text-[var(--text-primary)]' : ''}`}
